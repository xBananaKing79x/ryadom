...
export function MarketplaceApp() {
  ...
  const participantNameCache = useRef<Record<string, string>>({});

  const showToast = useCallback((text: string) => {
    setToast(text); if (toastTimer.current) clearTimeout(toastTimer.current); toastTimer.current = setTimeout(() => setToast(""), 3500);
  }, []);

  const hydrateParticipantProfiles = useCallback(async (profileIds: Array<string | undefined>) => {
    const uniqueIds = [...new Set(profileIds.filter((id): id is string => Boolean(id)))];
    const missingIds = uniqueIds.filter((id) => !participantNameCache.current[id]);
  ...
  const hydrateOrderImages = useCallback(async (list: Order[]) => {
    await Promise.all(list.slice(0, 20).map(async (order) => {
      const productId = order.product_id || order.product?.id;
      if (!productId) return;
      try {
        const images = await mcpCall<ImageRecord[]>("get_product_images", { product_id: productId });
        if (!images?.[0]?.url) return;
        setOrders((current) => current.map((item) => item.id === order.id && item.product ? { ...item, product: { ...item.product, image: images[0].url } } : item));
      } catch { /* Изображение необязательно. */ }
    }));
  }, []);

  const hydrateAgentOfferImages = useCallback(async (list: AgentOffer[]) => {
    await Promise.all(list.slice(0, 20).map(async (offer) => {
      try {
        const images = await mcpCall<ImageRecord[]>("get_product_images", { product_id: offer.product.id });
        if (!images?.[0]?.url) return;
        setAgentOffers((current) => current.map((item) => item.message.id === offer.message.id ? { ...item, product: { ...item.product, image: images[0].url } } : item));
      } catch { /* Изображение необязательно. */ }
    }));
  }, []);

  const hydrateAgentOrderImages = useCallback(async (list: Order[]) => {
    await Promise.all(list.slice(0, 20).map(async (order) => {
      const productId = order.product_id || order.product?.id;
      if (!productId) return;
      try {
        const images = await mcpCall<ImageRecord[]>("get_product_images", { product_id: productId });
        if (!images?.[0]?.url) return;
        setAgentOrders((current) => current.map((item) => item.id === order.id && item.product ? { ...item, product: { ...item.product, image: images[0].url } } : item));
      } catch { /* Изображение необязательно. */ }
    }));
  }, []);
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (!stopped && materialActions.length) {
          showToast(materialActions.some((action) => action.kind === "sold") ? "Агент завершил продажу" : materialActions.some((action) => action.kind === "payment_sent") ? "Агент отправил тестовый перевод" : "Агент обновил сделки");
    ...
    const timer = setTimeout(() => {
      ...
      void Promise.allSettled([
        ...
        if (messageResult.status === "fulfilled") {
          const list = Array.isArray(messageResult.value) ? messageResult.value : [];
          setDealMessages(list); void hydrateMessageProfiles(list);
        }
  ...
  }, [hydrateMessageProfiles, panel, selected]);
  ...
  async function openOrders(role = orderRole) {
    ...
    try {
      ...
      const orderList = await Promise.all(rawOrders.map(async (order) => {
        if (order.product || !order.product_id) return order;
        try { return { ...order, product: await mcpCall<Product>("get_product", { product_id: order.product_id }) }; }
        catch { return order; }
      }));
      setOrders(orderList); setSellingProducts(Array.isArray(products) ? products : []);
      void hydrateOrderImages(orderList);
  ...
  async function openMessages(context?: Partial<typeof messageContext>) {
    ...
    try {
      ...
      const list = await mcpCall<Message[]>("get_messages", args); const messageList = Array.isArray(list) ? list : [];
      setMessages(messageList); void hydrateMessageProfiles(messageList);
  ...
  async function sendDealMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDealMessages(messageList); void hydrateMessageProfiles(messageList); showToast("Сообщение продавцу отправлено");
  ...
  async function sendSepoliaPayment(payment: PaymentDetails | null = agentPayment, source: "agent" | "deal" = "agent") {
    ...
    try {
      ...
      const nextPayment = { ...payment, transaction: { hash, status: "pending" as const, finalized: false, recipient_matches: true, amount_matches: true, expected_amount_eth: payment.amount_eth, amount_eth: payment.amount_eth, explorer_url: explorerUrl } };
      ...
      if (source === "deal" && selected) {
        try {
          ...
          const list = await mcpCall<Message[]>("get_messages", { product_id: selected.id, limit: 100 });
          const messageList = Array.isArray(list) ? list : [];
          setDealMessages(messageList); void hydrateMessageProfiles(messageList);
  ...
  const messageAuthor = useCallback((message: Message) => message.sender_id === profile?.id ? "Вы" : message.sender_id ? participantNames[message.sender_id] || "Собеседник" : "Собеседник", [participantNames, profile?.id]);
  ...
  return <main id="top">
    ...
    {panel === "agent" && <Modal eyebrow="DeepSeek × MCP" title="Агент по сделкам" wide close={() => setPanel(null)}><div className="agent-shell">
      <aside className="agent-aside"><div className="agent-orb" aria-hidden="true">✦</div><strong>Что поручить?</strong><p>Агент ищет товары, торгуется с контрагентами и ведёт разрешённые сделки по заданным правилам.</p><div className="agent-prompts"><button onClick={() => void runAgentTask("Хочу найти товар. Спроси у меня категорию, бюджет и важные параметры.")} type="button">Найти товар</button><button onClick={() => void runAgentTask("Найди товары в статусе RESERVED — на холде. Сначала уточни, что именно искать.")} type="button">Поиск на холде</button><button onClick={() => void runAgentTask("Продолжи все мои покупки: торгуйся до минимальной или финальной цены, добейся резерва, запроси кошелёк и оплати тестовым SepoliaETH.")} type="button">Провести покупки</button><button onClick={() => void runAgentTask("Автономно обработай все текущие продажи: сообщения, торг, заказы, резерв и финализированные оплаты по правилу 90%.")} type="button">Провести продажи</button><button onClick={() => void runAgentTask("Проверь входящие сообщения по моим товарам без изменения заказов.")} type="button">Только проверить</button><button onClick={() => void runAgentTask("Покажи завершённые покупки, которые можно выставить на продажу с наценкой 15%.")} type="button">Перепродать +15%</button><button onClick={() => void runAgentTask("Покажи реквизиты для тестовой оплаты в Ethereum Sepolia.")} type="button">Тестовая оплата ETH</button></div><small>Перед оплатой агент обязательно проверяет статус RESERVED. Повторный перевод по тому же заказу блокируется хэшем в чате.</small></aside>
      <section className="agent-workspace"><div className="agent-chat" aria-live="polite" aria-relevant="additions" ref={agentChatRef} role="log">{agentMessages.map((message, index) => <article className={`agent-message ${message.role}`} key={`${message.role}-${index}`}><span>{message.role === "assistant" ? "Р" : "Вы"}</span><p>{message.content}</p></article>)}{agentSending && <article className="agent-message assistant thinking"><span>Р</span><p>Ищу и сравниваю…</p></article>}</div>
        ...
        {agentSortedOrders.length > 0 && <div className="agent-results"><div className="agent-results-head"><strong>Входящие заказы</strong><span>{agentSortedOrders.length}</span></div><div className="agent-order-list">{agentSortedOrders.map((order) => { const buyerName = fullProfileName(order.buyer_profile) || (order.buyer_id ? participantNames[order.buyer_id] : "") || "Пользователь"; return <article className={order.id === bestAgentOrderId ? "agent-order recommended" : "agent-order"} key={order.id}>{order.product ? <button className={`agent-order-product category-${order.product.category}`} aria-label={`Открыть карточку ${order.product.title}`} onClick={() => openProduct(order.product as Product)} type="button">{order.product.image ? <img src={order.product.image} alt={order.product.title} /> : <span>{categoryNames[order.product.category] || "Товар"}</span>}</button> : <div className="agent-order-product"><span>Заказ</span></div>}<div>{order.id === bestAgentOrderId && <span className="agent-recommendation">Рекомендация агента</span>}<span className="agent-customer-name">Заказчик · <strong>{buyerName}</strong></span><strong>{order.product?.title || `Заказ ${order.id.slice(0, 8)}`}</strong><small>{statusNames[order.status] || order.status} · {shortDate(order.created_at)}</small></div><div><strong>{order.product ? money(order.product.price) : "Цена объявления"}</strong>{order.status === "CREATED" && <button disabled={acting} onClick={() => void acceptAgentOrder(order)} type="button">Подтвердить</button>}</div></article>; })}</div><p className="agent-order-note">MCP не хранит отдельную цену предложения: сравнение выполняется по цене объявления. При равенстве агент не обещает несуществующую выгоду.</p></div>}
